...
# For the Waveshare 2" esp32-s3 Touch LCD
# see: https://files.waveshare.com/wiki/ESP32-S3-LCD-2/ESP32-S3-Touch-LCD-2-SchDoc.pdf
pins = {
    "qr_module": {
        "rx": 44,
        "tx": 43,
...
class QRHost:
    ...
    def init_uart(self, baudrate=9600):
        ...
        self.uart.init(
            ...
            parity=None,
            stop=1,
            rx=pins["qr_module"]["rx"],
            tx=pins["qr_module"]["tx"],
            rxbuf=2048,
            timeout=10  # Ensures that we don't think a tx is done before all data is sent
        )
...
#        b |= (1<<4)
    ...
    def configure(self):
        ...
        print("configure")
        self.uart.read()
        print("finished uart.read()")

        save_required = False
        val = self.get_setting(SERIAL_ADDR)
        print(f"SERIAL_ADDR: {val}")
        if val is None:
        ...
        print(f"INTERVAL_OF_SCANNING_ADDR: {val}")
        if val is None:
        ...
        print(f"DELAY_OF_SAME_BARCODES_ADDR: {val}")
        if val is None:
        ...
        if save_required:
            ...
            if not val:
                ...
                print("save_settings_on_scanner suceeded")
...
result = scanner.configure()
print(f"configure() returned {result}")

def update():
    while True:
        if scanner.uart.any() > 0:
            time.sleep(scanner.chunk_timeout)
            data = scanner.uart.read()
            scanner.uart.flush()
            if not scanner.uart.txdone():
                raise Exception("tx not done!!")
            if data:
                try:
                    print(data.decode() + "\n")
